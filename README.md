...
**Schedule 课程表插件** 是基于 [Yunzai-Bot V3](https://github.com/TimeRainStarSky/Yunzai) 的课程表管理插件。它支持通过~~WakeUP 课程表的口令~~、星链课表口令、JSON文件（支持本插件原生格式和[拾光课程表](https://github.com/XingHeYuZhuan/shiguangschedule)的导出文件）、ICS文件以及Wakeup备份文件等方式一键导入课表，并提供课表查询、群友上课状态围观、课表推送订阅、课表导出、快速调课等实用功能。
...
>
> **4月初**：迁移课表请求接口并加入鉴权，开发者尝试通过反代服务临时解决；
>
> **4月29日**：进一步加强鉴权，请求直接被拦截并返回 **“命中反作弊”**。
>
...
- **一键导入**：支持 ICS 文件、星链课表口令、拾光课程表JSON等多种方式导入，无需繁琐配置
- **跨群共享**：数据与 QQ 绑定，无需每个群单独设置
- **灵活查询**：按周数、日期查询，支持自然语言输入，今日/明日课表一目了然
- **群组互动**：围观群友上课状态，支持设置“翘课”模式
- **智能学期判断**：自动计算当前周数，学期结束友好提醒
- **定时推送**：订阅后每天推送明日课表（需加好友）
- **上课提醒**：开启后每节课开始前按设定提前量私信提醒（需加好友），阈值 5~60 分钟可调
- **个性设置**：自定义昵称、签名，支持自由调课，打造专属课表
...
| 命令 | 说明 |
|------|------|
| `#设置课表 星链课表分享口令` | 导入课程表（可直接发送包含「口令」的消息） |
| `#导入课表` | 发送JSON文件导入课表，支持插件格式和拾光格式 |
| `#导出课表(拾光)?` | 导出课表JSON文件，支持导出为拾光格式 |
| `#清除课表` | 清除自己的课表 |
| `#课表设置昵称 <昵称>` | 修改显示昵称（≤20字） |
| `#课表设置签名 <签名>` | 设置个性签名（≤30字） |
| `#设置学期开始 <日期>` | 手动修正学期开始日期（仅改日期、课程周次不变，适用于周次本就是校历周的课表） |
| `#对齐课表 <日期>` | 课表周次与校历整体偏移时使用（如开学第一个月没有课，10-05 被当成第 1 周而实际是校历第 5 周）。会把课程周次整体平移并对齐到真实校历开学日，上课日期保持不变 |
...
| `#更新节假日数据` | 手动检查并更新节假日数据（需主人权限），加`强制`前缀跳过每日一次的限制（如 `#强制更新节假日数据`）。节假日数据默认每日随课表推送定时任务自动更新，无需手动操作 |
| `生日模块` | 支持设置生日并推送，适配农历生日。详见`#生日帮助`菜单 |
...
├─ data                # 用户数据（课表、节假日自动更新数据等，不纳入 git）
├─ guoba               # 锅巴适配目录
│  └─ schemas          # 配置表单
...
> 💡 **课程门数统计口径**：导入回执中的"课程数量"与 `#我的课表` 的"总课程数/本周课程"按**课程名称去重**统计。同一门课因单双周拆分、中途更换地点等原因存在多条记录的只计 1 门（存储格式不变，仅影响统计），保证不同导入源统计结果一致。
